        ...
        <div class="max-w-[1800px] mx-auto space-y-6 pb-8">
            ...
            <div class="glass-effect rounded-3xl p-6 shadow-lg border border-gray-200 dark:border-gray-700">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    {{-- Filters --}}
                    <div
                        class="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 min-w-[320px]">
                        {{ $this->form }}
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="space-y-2 w-full">
                            <h3 class="text-sm font-semibold text-red-800 dark:text-red-200">
                                Schedule Conflicts Detected ({{ $this->conflicts->count() }})
                            </h3>
                            <div class="grid gap-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                                @foreach($this->conflicts as $conflict)
                                    <div
                                        class="text-xs bg-white dark:bg-white/5 p-2 rounded-lg border border-red-100 dark:border-red-500/30">
                                        <div class="font-medium text-red-700 dark:text-red-300">
                                            {{ $conflict['message'] }}
                                        </div>
                                        <div class="text-gray-500 dark:text-gray-400 mt-1">
                                            {{ $conflict['section_a']->course->code }} vs
                                            {{ $conflict['section_b']->course->code }}
                                            ({{ \Carbon\Carbon::parse($conflict['section_a']->start_time)->format('H:i') }})
                                        </div>
                                    </div>
                                @endforeach
                            </div>
            ...
            <div class="glass-effect rounded-3xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="overflow-x-auto">
                    <div class="inline-block min-w-full">
                        {{-- Header Row --}}
                        <div
                            class="time-grid bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                            {{-- Empty Corner Cell --}}
                            <div
                                class="sticky left-0 z-20 bg-gray-50 dark:bg-gray-800 p-4 border-r border-gray-200 dark:border-gray-700">
                            ...
                            @foreach ($this->getTimeSlots() as $slot)
                                <div class="p-4 border-r border-gray-200 dark:border-gray-700 last:border-r-0 text-center">
                                    ...
                                    <div
                                        class="inline-flex items-center px-3 py-1.5 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                        ...
                        @foreach ($this->getTimetableData() as $day => $sections)
                            <div
                                class="time-grid border-b border-gray-200 dark:border-gray-700 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                {{-- Day Label --}}
                                <div
                                    class="sticky left-0 z-20 bg-white dark:bg-gray-900 p-4 border-r border-gray-200 dark:border-gray-700">
                                ...
                                @foreach ($timeSlots as $index => $slotStart)
                                    ...
                                    <div
                                        class="p-3 border-r border-gray-200 dark:border-gray-700 last:border-r-0 min-h-[140px]">
                                        <div class="space-y-2">
                                            @foreach ($slotSections as $section)
                                                @php $style = $this->getCourseColor($section->course_id); @endphp

                                                <div
                                                    class="card-hover rounded-2xl p-4 {{ $style[0] }} {{ $style[1] }} border-2 {{ $style[4] }} shadow-md">
                                                    {{-- Course Code Badge --}}
                                                    <div
                                                        class="inline-flex items-center px-2.5 py-1 rounded-lg bg-white/90 dark:bg-black/20 border border-gray-200 dark:border-gray-700 mb-3">
                                                    ...
                                                    <div
                                                        ...
                                                        <span
                                                            class="inline-flex items-center px-2 py-0.5 rounded-md bg-gray-100 dark:bg-gray-800 text-xs font-semibold text-gray-700 dark:text-gray-300">
